defmodule ElixirFriends.PostControllerTest do
  ...
  test "shows an image for the post", %{conn: conn} do
    {:ok, post} = Repo.insert %Post{image_url: "https://clubfredbaja.files.wordpress.com/2013/10/la-la-la.jpg"}
    conn = get conn, post_path(conn, :show, post)
    assert html_response(conn, 200) =~ "Show post"
    assert html_response(conn, 200) =~ "<img src='https://clubfredbaja.files.wordpress.com/2013/10/la-la-la.jpg' />"
  end
